# ============================================================
# AiScript — Python-like scripting language
# Single-file interpreter: lexer, parser, AST, evaluator, REPL
# ============================================================
import sys as _sys, os as _os, random as _random, math as _math
...
class _Parser:
    ...
    def _func_def(self):
        ...
        params=[]
        if self._peek()!=_KW and self._peek()!="RPAREN" or self._peek_val()!="self":
            while self._peek()==_ID:
                params.append(self._expect(_ID).value)
                if self._peek()=="COMMA": self.idx+=1; self._skip_newlines()
                else: break
    ...
    def _del_stmt(self):
        ...
        return _Assign([_Ident(expr.name)],_None())
    def _expr_stmt(self):
        expr=self._expr()
...
# ---- REPL ----
def repl():
    e=_Eval(); print("AiScript v1.0.0 — type 'exit()' to quit")
    src=""
    while True:
        try:
